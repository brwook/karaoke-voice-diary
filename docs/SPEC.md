...
- 분석 큐: 동시 1개 순차 처리(MediaCodec 자원·CPU 보호). 대기 항목도 progress 맵에 0f로 등록되어 "분석 중 0%"로 표시. 큐는 메모리 전용이므로 **앱 시작 시 NOT_ANALYZED 녹음 전부를 자동 재큐잉**(`enqueueAllPending` — stale ANALYZING 복구 직후 실행). 파일 직접 선택 가져오기도 즉시 분석 시작.
