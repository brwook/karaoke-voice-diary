...
1. **HomeScreen (녹음 탭)**: 녹음 목록(이름, 가져온 날짜, 길이 mm:ss, 분석 상태 뱃지 / 분석 중이면 진행률). **곡 등록 완료 여부로 두 섹션 분리** (2026-07-07 추가): 상단 "정리할 녹음"(미분석·분석 중·실패·구간 0개·곡 등록 미완), 하단 "정리 완료"(분석 완료 + 모든 구간에 곡 배정). 섹션 헤더는 labelLarge(onSurfaceVariant)+개수, 항목 없는 섹션은 헤더 생략. 분석 완료 카드의 칩: 곡 등록 미완이면 NEUTRAL "곡 N/M", 전부 등록이면 SUCCESS "곡 M", 구간 0개면 NEUTRAL "구간 없음". FAB "가져오기" → SAF `OpenDocument(audio/*)` → `takePersistableUriPermission` → ContentResolver로 displayName, `MediaMetadataRetriever`로 duration(IO 스레드) → `importRecording`. 항목 탭 → 상세. 길게/삭제 버튼 → 확인 다이얼로그 후 삭제.
