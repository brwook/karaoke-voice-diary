...
앱은 이미 구간·별점 데이터를 갖고 있으므로, "별점 4+ 테이크를 wav로 내보내기" 기능(하루 작업)만 붙이면 학습 데이터 준비가 자동화된다.

---

## 8. 서비스 방향 검토 (2026-07 조사)

"나중에 서비스한다면 어디서 돌릴 것인가"에 대한 아키텍처 비교. 핵심 변수는 기술이 아니라 **저작권**이다 — 개인이 자기 기기에서 만드는 건 사적 이용 범위지만, 서비스가 원곡 파일을 서버에서 복제·처리하는 순간 사업자가 저작권 리스크를 정면으로 진다.

### 온디바이스 실행 가능성 (조사 결과)

- **음원 분리**: Demucs v4의 안드로이드 포팅이 이미 존재 — demucs.cpp 기반 NDK 앱(sevagh/demucs-android), Demixr 앱, 2026년 HT-Demucs ONNX 공개 export (CPU에서 PyTorch 대비 1.31배 빠름). 현재 안드로이드는 CPU 추론 위주 → 플래그십에서 곡당 수 분 수준. **가능, 단 느림.**
- **음색 변환(RVC 추론)**: ONNX export 공식 지원(HuBERT ~293MB + net_g ~110MB, 양자화로 축소 가능), PC에선 실시간 스트리밍 사례 다수, 모바일 온디바이스(ONNX Runtime/CoreML) 실증 사례 존재. **플래그십 기준 가능성 높음, 실측 필요.**
- **학습**: 온디바이스 불가. 클라우드/PC GPU 필수 (사용자당 1회, 30~60분).

### 아키텍처 옵션 비교

| | 개발 난이도 | 운영비 | 저작권 리스크 | 프라이버시 | 판정 |
|---|---|---|---|---|---|
| A. 완전 온디바이스 (학습까지) | 불가능 | — | — | — | ❌ |
| **B. 하이브리드: 학습만 클라우드 1회, 분리·변환은 온디바이스** | 높음 | 사용자당 1회 GPU 수십 분 (수백 원) | **최소** — 원곡이 기기 밖으로 안 나감 | 목소리 스템만 1회 업로드 | ⭐ **권장** |
| C. 풀 서버 (모든 처리 서버) | 중간 | 곡당 GPU 변동비 | **높음** — 원곡 업로드/복제 | 목소리+음원 서버 보관 | 법적 검토 전엔 위험 |
| D. 서드파티 API (Kits.AI 등) | 낮음 | 구독/호출당 종속 | 위탁하지만 잔존 | 외부 이전 | 빠른 실험용만 |

### 권장 로드맵

1. **0단계 (지금)**: GPU PC에서 품질 PoC (§4) — "코노 녹음에서 뽑은 음색이 쓸만한가"가 모든 것의 전제. 여기서 실패하면 아키텍처 논의 무의미.
2. **1단계 (하이브리드 기술 검증)**: ① RVC 모델을 Colab에서 학습 → ONNX 변환 → 안드로이드 테스트 앱에서 ONNX Runtime로 곡당 변환 시간 실측 (S23+ 기준), ② demucs.cpp 온디바이스 분리 시간 실측. 이 두 숫자가 하이브리드의 기술 리스크 전부다.
3. **2단계 (앱 통합)**: "내 음색 만들기"(서버 1회, 별점 높은 테이크 자동 업로드) + "커버 만들기"(온디바이스 큐). **기존 인프라 재활용도가 높다** — 분석 큐/Foreground Service/구간·별점 데이터가 커버 생성 큐에 그대로 쓰인다.
4. **과금 구조 (자연스러움)**: 음색 모델 생성(서버 비용 발생분)에만 과금, 커버 생성은 온디바이스라 무제한 무료 — 비용과 가격이 정직하게 일치.

참고 자료: demucs-android(github.com/sevagh/demucs-android), Demixr(github.com/demixr/demixr-app), HT-Demucs ONNX export(stemsplit.io, 2026-05), Mixxx GSoC 2025 Demucs→ONNX, RVC ONNX 모바일 사례(web.navan.dev, 2025-10).
